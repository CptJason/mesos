...
TEST(ReservedResourcesTest, Validation)
{
  // Unreserved.
  EXPECT_NONE(
      Resources::validate(createReservedResource("cpus", "8", "*", None())));

  // Statically role reserved.
  EXPECT_NONE(
      Resources::validate(createReservedResource("cpus", "8", "role", None())));
...
TEST(ReservedResourcesTest, Equals) {
  std::vector<Resources> unique = {
    // Unreserved.
    createReservedResource("cpus", "8", "*", None()),
    // Statically reserved for role.
    createReservedResource("cpus", "8", "role1", None()),
    createReservedResource("cpus", "8", "role2", None()),
...
TEST(ReservedResourcesTest, Addition) {
  // Statically reserved for role.
  {
