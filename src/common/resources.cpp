...
{
  // NOTE: Not setting the DiskInfo is the same as setting it as None,
  // therefore we just call .disk() even if it's not set.
...
static bool addable(const Resource& left, const Resource& right)
{
  if (left.name() != right.name() ||
      ...
      left.role() != right.role() ||
      left.has_reservation() != right.has_reservation() ||
      left.disk().has_persistence() || right.disk().has_persistence()) {
    return false;
  }
